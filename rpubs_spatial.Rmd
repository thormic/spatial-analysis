...
## Hypothesis
In furhter analysis we decided to verify 2 major hypotheses:
...
- mean of house prices in OAs is affected by socio-economic structure of this particular and neighbouring OAs
...
We set the proximity to 1000 and observe hot-spots based on intenisty of clusters, on the map below:
...
Let's see if we can infer more from regression models and explain the employed indicator.

## Linear model - employed

We will start with OLS model.
...
plot(model_sig_2)
```
We obtain almost ideally fitted model, with R-squared = 0.99. It means that the variables that we chose explain it very accurately. We can further explore the coefficients, all of them are positive, however qualifications level is the strongest one. Indicator that people lives alone being the weakest. We can also observe that both white and black races impact the employed indicator in almost the same way.
...
There are no significant patterns, so we may expect that there are no variables that are unobserved in our analysis. However, spatial analysis using GWR may bring more information.
...
R-squared seemed to be much lower, let's see how it looks divided among the areas. 
...
However, as in the linear model, all coefficients are positive. We can further explore them divided among OAs.
