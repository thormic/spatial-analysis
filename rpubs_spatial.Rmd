...
locations <-SpatialPointsDataFrame(house_data[,6:7], house_data,
                                      proj4string = CRS("+init=EPSG:27700"))

hist_df <- gather(data[,-1], key = "name", value = "value")

output_areas <- readOGR("data/statistical-gis-boundaries-london/ESRI", "OA_2011_London_gen_MHW")
output_areas <- output_areas[output_areas$LAD11NM=="Kensington and Chelsea",]

oa_data <- merge(output_areas, data, by.y ="OA", by.x="OA11CD")
proj4string(oa_data) <- CRS("+init=EPSG:27700")

house_data_agg <- house_data %>%
  group_by(oa11) %>%
  dplyr::summarize(mean_price = mean(price_paid, na.rm=TRUE))

houses_merged <-  data %>%
  inner_join(house_data_agg, by = c("OA" = "oa11"))

oa_data.mp <- merge(output_areas, houses_merged, by.y ="OA", by.x="OA11CD", all = FALSE)
proj4string(oa_data.mp) <- CRS("+init=EPSG:27700")
```
...
boxplot(data$christian, data$muslim, data$jewish,
        data$no_religion, ylim=c(0,100),
...
boxplot(data$white, data$black_african, data$asian,
        data$other_arab, ylim=c(0,100),
        col = "dodgerblue", rectCol="dodgerblue3", colMed="dodgerblue4",
...
boxplot(data$employed, data$unemployed, data$kids_english, data$male, data$white,
        data$Age_30_44, data$married, data$owned,  ylim=c(0,100),
        col = "dodgerblue", rectCol="dodgerblue3", colMed="dodgerblue4",
...
p <- ggplot(data, aes(unemployed, highest_quali))
p + geom_point(aes(colour = muslim, size = black_african)) 

p2 <- ggplot(data, aes(highest_quali, employed))
...
tm_shape(oa_data) + tm_fill("highest_quali", palette = "Purples", style = "quantile",
                              title = "% with a highest Qualifications") + tm_borders(alpha=.4)

tm_shape(oa_data) + tm_fill("black_african", palette = "YlGn", style = "quantile",
                              title = "% of Black/African") + tm_borders(alpha=.4)

tm_shape(oa_data) + tm_fill("muslim", palette = "GnBu", style = "quantile",
                              title = "% of Muslim") + tm_borders(alpha=.4)

tm_shape(oa_data) + tm_fill("white", palette = "PuBu", style = "quantile",
                              title = "% of White") + tm_borders(alpha=.4)

tm_shape(oa_data) + tm_fill("united_kingdom", palette = "BuGn", style = "quantile",
...
tm_shape(oa_data) + tm_fill("other_eu", palette = "Greens",
                              style = "quantile", title = "% of citizens born in EU") +
  tm_borders(alpha=.4) +
  tm_shape(locations) + tm_bubbles(col = "price_paid", size = 0.2,
...
tm_shape(oa_data) + tm_fill("unemployed", palette = "Oranges",
...
kde.output <- kernelUD(locations, h="href", grid = 1000)
kde <- raster(kde.output)
projection(kde) <- CRS("+init=EPSG:27700")
tmap_mode("plot")
bounding_box <- bbox(output_areas)
masked_kde <- mask(kde, output_areas)
```
...
kernel1 <- tm_shape(masked_kde, bbox = bounding_box) + tm_raster("ud", style = "quantile",
                                                      ...
                                                      legend.show = FALSE,
                                                      palette = "YlGnBu") +
  tm_shape(output_areas) + tm_borders(alpha=.3, col = "white") +
...
kernel2 <- tm_shape(output_areas) + tm_fill(col = "#f0f0f0") + tm_borders(alpha=.8, col = "white") +
...
neighbours <- poly2nb(oa_data)
neighbours2 <- poly2nb(oa_data, queen = FALSE)

# plot(oa_data, border = 'lightgrey')
# plot(neighbours, coordinates(oa_data), add=TRUE, col='blue')
# plot(neighbours2, coordinates(oa_data), add=TRUE, col='red')
...
moran.test(oa_data$employed, listw)
```
<br>
...
local <- localmoran(x = oa_data$employed,
                    listw = nb2listw(neighbours2, style = "W"))
moran.map <- cbind(oa_data, local)
...
m.employed <- oa_data$employed - mean(oa_data$employed)
...
plot(oa_data,border="lightgray",col=colors[findInterval(quadrant,brks,all.inside=FALSE)])
...
nb <- dnearneigh(coordinates(oa_data),0,750)
nb_lw <- nb2listw(nb, style = 'B')


# Gi statistic
local_g <- localG(oa_data$employed, nb_lw)
local_g <- cbind(oa_data, as.matrix(local_g))
...
model_sig_2 <- lm(oa_data$employed ~ .-1, data = oa_data[,sig_cols_2])
summary(model_sig_2)
...
map.resids <- cbind(oa_data, resids)
...
GWRbandwidth <- gwr.sel(oa_data$employed ~ .-1, data = oa_data[,sig_cols_2], adapt =TRUE)
```
...
gwr.model = gwr(oa_data$employed ~ .-1,
                data = oa_data[,sig_cols_2], adapt=GWRbandwidth, hatmatrix=TRUE, se.fit=TRUE)
gwr.model
```
...
gwr.map <- cbind(oa_data[,sig_cols_2], as.matrix(results))
...
model_sig <- lm(oa_data.mp$mean_price ~ ., data = oa_data.mp[sig_cols])
summary(model_sig)
...
map.resids <- cbind(oa_data.mp, resids)
names(map.resids)[58] <- "resids"
tm_shape(map.resids) + tm_fill("resids", palette="Blues") + tm_borders(alpha=.4) + tm_layout(frame = FALSE)
```
<br>
...
GWRbandwidth <- gwr.sel(oa_data.mp$mean_price ~ ., data = oa_data.mp[,sig_cols], adapt =TRUE)
```
...
gwr.model = gwr(oa_data.mp$mean_price ~ .,
                data = oa_data.mp[,sig_cols], adapt=GWRbandwidth, hatmatrix=TRUE, se.fit=TRUE)
gwr.model
...
results <-as.data.frame(gwr.model$SDF)
gwr.map <- cbind(oa_data.mp[,sig_cols], as.matrix(results))
tm_shape(gwr.map) + tm_fill("localR2", palette="PuBuGn")+ tm_borders(alpha=.4) + tm_layout(frame = FALSE)
```
<br>
...
grid <-spsample(locations, type = 'regular', n = 20000)
idw <- idw(locations$price_paid ~ 1, locations, newdata= grid)
...
masked_idw <- mask(raster_idw, output_areas)
# plots the masked raster
tm_shape(masked_idw) + tm_raster("prediction", style = "quantile", n = 100,
                                 legend.show = FALSE) +
  tm_shape(locations) + tm_bubbles(size = "price_paid", col = "price_paid",
                                      palette = "Blues", style = "quantile",
                                      legend.size.show = FALSE,
                                      title.col = "Price Paid (£)") +
