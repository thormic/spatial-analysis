...
tm_shape(gwr.map) + tm_fill("localR2", palette="PuBuGn")+ tm_borders(alpha=.4) + tm_layout(frame = FALSE)
```
This time, GWR clearly fits better on north-west part and gradually lowers the quality of fit further going east.
...
- On the other hand, OAs with higher denisty of Asians in the north will be negatively impacted the most, similar to single people density.
...
We are now going to try different technique, called interpolation. We will first need to create Thiessen polygons, that actually creates areas similar to our OAs, by assigning boundaries to the closest housing point. We then clip those boundaries to the area of Kensington and Chelsea and finally plot them on the map, filling the areas with price levels.
